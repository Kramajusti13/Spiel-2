...
import { hasSave, saveSummary, takeResetNotice, takeQuestResetNotice } from '../save.js';
...
export class MainMenu {
  constructor(game) {
    ...
    /**
     * Hinweis, dass ein Stand aus einer aelteren Version zurueckgesetzt wurde
     * (Erweiterung 2, Abschnitt 6). Bleibt stehen, solange das Menue offen
     * ist — er ist die einzige Erklaerung dafuer, dass "Weiterspielen" fehlt.
     */
  ...
  open() {
    this.age = 0;
    this.confirming = false;
    // Reihenfolge: erst lesen, dann den Hinweis abholen — saveSummary() ist
    // der Aufruf, bei dem ein Stand mit alter Versionsnummer auffliegt.
    this.summary = saveSummary();
    this.resetNotice = takeResetNotice() ?? takeQuestResetNotice() ?? this.resetNotice;
    this.build();
    this.layout();
  ...
  draw(ctx) {
    ...
    } else if (this.resetNotice) {
      // Ohne diese Zeilen waere ein fehlender "Weiterspielen"-Knopf oder eine
      // leere Questliste ein Raetsel. Der Grund steht deshalb dabei.
      drawText(ctx, this.resetNotice.text, VIEW.width / 2, this.panelY - 30,
        COLORS.menuAccent, UI.hud.fontSmall, 'center', 'middle');
      drawText(ctx, this.resetNotice.reason, VIEW.width / 2, this.panelY - 14,
    ...
    const lines = [
      ...
      'Rechtsklick blocken · Leertaste rollen · 1/2 Waffe · F Speerwurf',
