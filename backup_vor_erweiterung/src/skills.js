 ...
 *
 * Punkte kommen aus getoeteten Monstern (1 pro 15 Kills, spieluebergreifend) und
...
export function earnedPoints(progress, kills) {
  return Math.floor(kills / SKILLS.killsPerPoint) + (progress.bonusPoints ?? 0);
}

/** Kills bis zum naechsten Punkt. */
export function killsToNextPoint(kills) {
  return SKILLS.killsPerPoint - (kills % SKILLS.killsPerPoint);
}
